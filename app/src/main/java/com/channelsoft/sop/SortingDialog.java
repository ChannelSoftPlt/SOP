...
public class SortingDialog extends DialogFragment implements View.OnClickListener {
    ...
    private AppCompatSpinner branch;
    ...
    private void objectSetting() {
        ...
        if (getArguments() != null) {
            ...
            fetchBranch(bundle.getString("branch_id"));
        }
    }
    ...
    public void fetchBranch(final String branchID) {
                                ...
                                getActivity().runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        try {
                                            ...
                                            selectExistingPosition(branchID);
    ...
    private void selectExistingPosition(String branchID) {
        for (int i = 0; i < branchObjectArrayList.size(); i++) {
            if (branchObjectArrayList.get(i).getId().equals(branchID)) {
                branch.setSelection(i);
                return;
            }
        }
    }
    ...
    private void applySorting() {
        sortingDialogCallBack.applySorting(tvDateStart.getText().toString(), tvDateEnd.getText().toString(), branchObjectArrayList.get(branch.getSelectedItemPosition()).getId());
        dismiss();
    }

    public interface SortingDialogCallBack {
        void applySorting(String dateStart, String dateEnd, String branchID);
    }
}
